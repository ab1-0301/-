...
def site_detail(id):
    ...
    inventory = conn.execute("""
        ...
        WHERE i.site_id = ? AND i.quantity > 0 ORDER BY m.code
    """, (id,)).fetchall()
    worker_count = conn.execute("SELECT COUNT(*) AS cnt FROM workers WHERE site_id=?", (id,)).fetchone()["cnt"]
    today_expenses = conn.execute("""
        SELECT * FROM expenses WHERE site_id=? AND date(created)=? ORDER BY created
    """, (id, today)).fetchall()
    today_expenses_total = sum(e["amount"] for e in today_expenses)
    workers_today = conn.execute("""
        SELECT w.id, w.name, a.check_in, a.status AS att_status
        FROM workers w LEFT JOIN attendance a ON a.worker_id = w.id AND a.date = ?
        WHERE w.site_id = ? ORDER BY w.name
    """, (today, id)).fetchall()
    outbound_today = conn.execute("""
        SELECT t.*, m.name, m.code FROM transactions t JOIN materials m ON m.id = t.material_id
        WHERE t.site_id = ? AND t.type = '出库' AND date(t.time) = ? ORDER BY t.time
    """, (id, today)).fetchall()
    conn.close()
    return render_template("site_detail.html", site=site, inventory=inventory,
                          worker_count=worker_count, today=today,
...
@app.route("/workers-global")
def workers_global():
    conn = get_db()
    today = datetime.now().strftime("%Y-%m-%d")
    rows = conn.execute("""
        SELECT w.id, w.name, w.phone, w.site_id, s.name AS site_name,
               a.check_in, a.status AS att_status
        FROM workers w
    ...
    for w in rows_to_list(rows):
        sn = w["site_name"]
        if sn not in groups:
            groups[sn] = {"site_name": sn, "workers": []}
        groups[sn]["workers"].append(w)
        total += 1
        if w["att_status"] == "旷工":
            absent += 1
        elif w["check_in"]:
            present += 1
    return render_template("workers_global.html", groups=list(groups.values()),
                           summary={"total": total, "present": present, "absent": absent, "none": total - present - absent},
                           today=today)
...
def worker_list(site_id):
    ...
    workers_attendance = [{"worker": dict(w), "record": dict(record_map[w["id"]]) if w["id"] in record_map else None} for w in workers]
    # 打卡历史
    ...
    return render_template("workers.html", site=site, workers=workers,
                           workers_attendance=workers_attendance, history=history, today=today)
...
@app.route("/site/<int:site_id>/expenses", methods=["GET", "POST"])
def expenses(site_id):
    conn = get_db()
    site = conn.execute("SELECT * FROM sites WHERE id=?", (site_id,)).fetchone()
    if not site:
        flash("项目地不存在", "error")
    ...
    if request.method == "POST":
        ...
        if not category:
            flash("请选择开支类别", "error")
            conn.close()
            return redirect(url_for("expenses", site_id=site_id))
        try:
        ...
        conn.execute(
            "INSERT INTO expenses (site_id, category, amount, description, operator, note, created) VALUES (?,?,?,?,?,?,?)",
        ...
        conn.commit()
        flash(f"开支已添加：{category} ¥{amount:.2f}", "success")
        conn.close()
        return redirect(url_for("expenses", site_id=site_id))

    expenses_list = conn.execute("""
        SELECT * FROM expenses WHERE site_id=? ORDER BY id DESC LIMIT 200
    """, (site_id,)).fetchall()
    total = conn.execute(
        "SELECT COALESCE(SUM(amount),0) AS total FROM expenses WHERE site_id=?",
        (site_id,)
    ).fetchone()["total"]
    conn.close()
    return render_template("expenses.html", site=site, expenses=expenses_list, total=total)


@app.route("/site/<int:site_id>/expense/<int:expense_id>/delete")
def expense_delete(site_id, expense_id):
    conn = get_db()
    conn.execute("DELETE FROM expenses WHERE id=? AND site_id=?", (expense_id, site_id))
    conn.commit()
    conn.close()
    flash("开支记录已删除", "success")
    return redirect(url_for("expenses", site_id=site_id))
...
def all_expenses():
    ...
    for s in sites:
        exps = conn.execute("""
            SELECT * FROM expenses WHERE site_id=? ORDER BY id DESC LIMIT 100
        """, (s["id"],)).fetchall()
        if exps:
            total = conn.execute(
                "SELECT COALESCE(SUM(amount),0) AS total FROM expenses WHERE site_id=?",
            ...
            expenses_by_site[s["name"]] = {"site": s, "expenses": exps, "total": total}
    conn.close()
    return render_template("all_expenses.html", expenses_by_site=expenses_by_site)




@app.route("/worker/<int:id>/monthly")
def worker_monthly(id):
    conn = get_db()
    worker = conn.execute("""
        SELECT w.*, s.name AS site_name
        FROM workers w JOIN sites s ON s.id = w.site_id
        WHERE w.id = ?
    """, (id,)).fetchone()
    if not worker:
        flash("工人不存在", "error")
    ...
    records = conn.execute("""
        ...
        ORDER BY date
    """, (id, f"{prefix}%")).fetchall()
    conn.close()
    return render_template("worker_monthly.html", worker=worker, records=records, year=year, month=month)
...
def api_sites():
    ...
    sites = conn.execute("""
        SELECT s.*,
               ...
               COUNT(DISTINCT i.material_id) AS total_types,
               (SELECT COUNT(*) FROM workers WHERE site_id = s.id) AS total_workers
        FROM sites s LEFT JOIN inventory i ON i.site_id = s.id
        GROUP BY s.id ORDER BY s.id
    """).fetchall()
    conn.close()
    return jsonify({"code": 0, "data": rows_to_list(sites)})


@app.route("/api/site/<int:id>")
def api_site_detail(id):
    conn = get_db()
    site = conn.execute("SELECT * FROM sites WHERE id=?", (id,)).fetchone()
    if not site:
        conn.close()
        return jsonify({"code": 1, "msg": "项目地不存在"})
    today = datetime.now().strftime("%Y-%m-%d")
    inventory = conn.execute("""
        SELECT i.*, m.code, m.name, m.spec, m.unit
        FROM inventory i JOIN materials m ON m.id = i.material_id
    ...
    return jsonify({"code": 0, "data": {
        ...
        "today": today,
        "today_expenses": rows_to_list(today_expenses),
        "today_expenses_total": today_expenses_total,
        "workers_today": rows_to_list(workers_today),
        "outbound_today": rows_to_list(outbound_today)
    }})
...
def api_materials():
    conn = get_db()
    mats = conn.execute("SELECT * FROM materials ORDER BY code").fetchall()
    conn.close()
    return jsonify({"code": 0, "data": rows_to_list(mats)})
...
def api_material_add():
    ...
    try:
        conn.execute("INSERT INTO materials (code, name, spec, unit) VALUES (?,?,?,?)",
                     (code, name, request.json.get("spec", "").strip(),
                      request.json.get("unit", "个").strip()))
        conn.commit()
        conn.close()
        return jsonify({"code": 0, "msg": "添加成功"})
    except sqlite3.IntegrityError:
        conn.close()
        return jsonify({"code": 1, "msg": f"编码「{code}」已存在"})


@app.route("/api/material/<int:id>/delete", methods=["POST"])
def api_material_delete(id):
    conn = get_db()
    conn.execute("DELETE FROM materials WHERE id=?", (id,))
    conn.commit()
    conn.close()
...
@app.route("/api/site/<int:site_id>/inbound", methods=["POST"])
def api_inbound(site_id):
    conn = get_db()
    site = conn.execute("SELECT * FROM sites WHERE id=?", (site_id,)).fetchone()
    if not site:
        conn.close()
        return jsonify({"code": 1, "msg": "项目地不存在"})
    material_id = int(request.json["material_id"])
    qty = int(request.json["quantity"])
    unit_price = float(request.json.get("unit_price", 0) or 0)
    operator = request.json.get("operator", "").strip()
    note = request.json.get("note", "").strip()
    if qty <= 0:
        conn.close()
        return jsonify({"code": 1, "msg": "数量必须大于0"})
    existing = conn.execute("SELECT * FROM inventory WHERE site_id=? AND material_id=?",
                            (site_id, material_id)).fetchone()
    if existing:
        conn.execute("UPDATE inventory SET quantity=quantity+? WHERE site_id=? AND material_id=?",
...
@app.route("/api/site/<int:site_id>/outbound", methods=["POST"])
def api_outbound(site_id):
    conn = get_db()
    site = conn.execute("SELECT * FROM sites WHERE id=?", (site_id,)).fetchone()
    if not site:
        conn.close()
    ...
    conn.commit()
    conn.close()
    return jsonify({"code": 0, "msg": "出库成功"})


@app.route("/api/site/<int:site_id>/records")
def api_site_records(site_id):
    conn = get_db()
    site = conn.execute("SELECT * FROM sites WHERE id=?", (site_id,)).fetchone()
    if not site:
        conn.close()
    ...
    records = conn.execute("""
        ...
        LEFT JOIN workers w ON w.id = t.worker_id
        WHERE t.site_id = ? ORDER BY t.id DESC LIMIT 200
    """, (site_id,)).fetchall()
    conn.close()
    return jsonify({"code": 0, "data": rows_to_list(records)})
...
def api_all_records():
    conn = get_db()
    records = conn.execute("""
        SELECT t.*, m.code, m.name, m.unit, s.name AS site_name, w.name AS worker_name
        FROM transactions t JOIN materials m ON m.id = t.material_id
...
@app.route("/api/site/<int:site_id>/workers")
def api_workers(site_id):
    conn = get_db()
    workers = conn.execute("SELECT * FROM workers WHERE site_id=? ORDER BY name", (site_id,)).fetchall()
    conn.close()
    return jsonify({"code": 0, "data": rows_to_list(workers)})


@app.route("/api/site/<int:site_id>/worker/add", methods=["POST"])
def api_worker_add(site_id):
    name = request.json.get("name", "").strip()
    if not name:
        return jsonify({"code": 1, "msg": "姓名不能为空"})
    conn = get_db()
    conn.execute("INSERT INTO workers (site_id, name, phone, id_card, created) VALUES (?,?,?,?,?)",
                 (site_id, name, request.json.get("phone", "").strip(),
                  request.json.get("id_card", "").strip(),
                  datetime.now().strftime("%Y-%m-%d")))
    conn.commit()
    conn.close()
    return jsonify({"code": 0, "msg": "添加工人成功"})


@app.route("/api/worker/<int:id>/delete", methods=["POST"])
def api_worker_delete(id):
    conn = get_db()
...
@app.route("/api/site/<int:site_id>/attendance")
def api_attendance(site_id):
    conn = get_db()
    today = datetime.now().strftime("%Y-%m-%d")
    workers = conn.execute("SELECT * FROM workers WHERE site_id=? ORDER BY name", (site_id,)).fetchall()
    today_records = conn.execute("""
        SELECT a.*, w.name AS worker_name
        FROM attendance a JOIN workers w ON w.id = a.worker_id
        WHERE a.site_id = ? AND a.date = ? ORDER BY w.name
    """, (site_id, today)).fetchall()
    history = conn.execute("""
        SELECT a.*, w.name AS worker_name
        FROM attendance a JOIN workers w ON w.id = a.worker_id
        WHERE a.site_id = ? ORDER BY a.id DESC LIMIT 50
    """, (site_id,)).fetchall()
    record_map = {r["worker_id"]: r for r in today_records}
    workers_attendance = []
    for w in workers:
        ...
        workers_attendance.append({
            "worker": row_to_dict(w),
            "record": row_to_dict(rec)
        })
    conn.close()
    return jsonify({"code": 0, "data": {
        "workers_attendance": workers_attendance,
...
def api_attendance_checkin():
    site_id = int(request.json["site_id"])
    ...
    if existing:
        ...
        conn.execute("UPDATE attendance SET check_in=?, note=? WHERE id=?", (now, note, existing["id"]))
    else:
        conn.execute("INSERT INTO attendance (worker_id, site_id, date, check_in, note) VALUES (?,?,?,?,?)",
                     (worker_id, site_id, today, now, note))
    conn.commit()
    conn.close()
...
def api_attendance_batch():
    ...
    note = request.json.get("note", "").strip()
    if not worker_ids:
        return jsonify({"code": 1, "msg": "请选择工人"})
    conn = get_db()
    today = datetime.now().strftime("%Y-%m-%d")
    now = datetime.now().strftime("%H:%M")
    count = 0
    for wid in worker_ids:
        ...
        if action == "checkin":
            ...
            if existing:
                conn.execute("UPDATE attendance SET status='旷工', note=? WHERE id=?", (note, existing["id"]))
            else:
                conn.execute("INSERT INTO attendance (worker_id, site_id, date, status, note) VALUES (?,?,?,?,?)",
                             (wid, site_id, today, '旷工', note))
            count += 1
    conn.commit()
    conn.close()
    return jsonify({"code": 0, "msg": f"操作完成：{count}人"})


@app.route("/api/workers/overview")
def api_workers_overview():
    conn = get_db()
    today = datetime.now().strftime("%Y-%m-%d")
...
@app.route("/api/worker/<int:id>/monthly")
def api_worker_monthly(id):
    ...
    if not worker:
        conn.close()
        return jsonify({"code": 1, "msg": "工人不存在"})
    records = conn.execute("""
        SELECT * FROM attendance
        WHERE worker_id=? AND date LIKE ?
        ORDER BY date
...
@app.route("/api/site/<int:site_id>/expenses")
def api_expenses(site_id):
    conn = get_db()
    expenses_list = conn.execute(
        "SELECT * FROM expenses WHERE site_id=? ORDER BY id DESC", (site_id,)
    ).fetchall()
    total = conn.execute(
        "SELECT COALESCE(SUM(amount),0) AS total FROM expenses WHERE site_id=?",
        (site_id,)
    ...
    conn.close()
    return jsonify({"code": 0, "data": {"expenses": rows_to_list(expenses_list), "total": total}})


@app.route("/api/site/<int:site_id>/expenses/add", methods=["POST"])
def api_expenses_add(site_id):
    data = request.json
    category = (data.get("category") or "").strip()
    try:
        amount = float(data.get("amount", 0))
    except (TypeError, ValueError):
        return jsonify({"code": 1, "msg": "金额无效"})
    if not category or amount <= 0:
        return jsonify({"code": 1, "msg": "类别和金额不能为空"})
    ...
    conn.execute(
        ...
        (site_id, category, amount,
         ...
         (data.get("operator") or "").strip(),
         (data.get("note") or "").strip(),
         datetime.now().strftime("%Y-%m-%d"))
    )
    conn.commit()
    conn.close()
    return jsonify({"code": 0, "msg": "开支记录已添加"})
